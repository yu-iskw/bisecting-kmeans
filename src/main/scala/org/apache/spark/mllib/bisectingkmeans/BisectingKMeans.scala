...
object BisectingKMeans extends Logging {
  ...
  private[bisectingkmeans] val ROOT_INDEX_KEY: BigInt = 1
...
class BisectingKMeans private(
  ...
  def run(input: RDD[Vector]): BisectingKMeansModel = {
    ...
    var clusterStats = mutable.Map.empty[BigInt, BisectingClusterNodeStat]
    var step = 1
  ...
  def calcCriterions(
    ...
    stats: Map[BigInt, BisectingClusterNodeStat]): Map[BigInt, BisectingClusterNode] = {
   ...
   */
  private[bisectingkmeans]
  def summarize(data: RDD[(BigInt, BV[Double])]): Map[BigInt, BisectingClusterNodeStat] = {
    ...
    stats.map { case (i, stat) =>
      i -> new BisectingClusterNodeStat(stat._2.toLong, stat._1, stat._3)
    }
  }
  ...
  def getDividedClusters(data: RDD[(BigInt, BV[Double])],
    leafClusters: Map[BigInt, BisectingClusterNodeStat]): Map[BigInt, BisectingClusterNodeStat] = {
    ...
    if (dividableClusters.isEmpty) {
      ...
      return Map.empty[BigInt, BisectingClusterNodeStat]
    }
  ...
  def divide(
    data: RDD[(BigInt, BV[Double])],
    currentStats: Map[BigInt, BisectingClusterNodeStat]): Map[BigInt, BisectingClusterNodeStat] = {
  ...
  def initChildCenters(
      data: RDD[(BigInt, BV[Double])],
      stats: Map[BigInt, BisectingClusterNodeStat]): Map[BigInt, BV[Double]] = {
  ...
  def updateClusterIndex(
    data: RDD[(BigInt, BV[Double])],
    dividedClusters: Map[BigInt, BisectingClusterNodeStat]): RDD[(BigInt, BV[Double])] = {
...
private[this] case class BisectingClusterNodeStat(
  rows: Long,
  sums: BV[Double],
